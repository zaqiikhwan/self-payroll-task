...
func (p *userRepository) FindByID(ctx context.Context, id int) (*model.User, error) {
	...
	if err := p.Cfg.Database().Preload("Position").WithContext(ctx).Where("id = ?", id).First(user).Error; err != nil {
		return nil, err
	}
...
func (p *userRepository) UpdateByID(ctx context.Context, id int, user *model.User) (*model.User, error) {
	...
	if err := p.Cfg.Database().WithContext(ctx).Model(&model.Position{ID: id}).Updates(user).Find(user).Error; err != nil {
		return nil, err
	}
	// panic("implement me ")
	return user, nil
